...
The primary key in the customers table would be the 'CustomerName' and orders table would be the 'ProductNumber'.  The foreign key would be the transaction number because it is unique to that customer and that order. The two tables would be related through the foreign key because it is the one thing that both tables have in common, but it is not repeated for any items in the table. 
